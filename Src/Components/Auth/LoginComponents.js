    ...
    componentWillReceiveProps(newProps) {
        setTimeout(() => {
            if (newProps.auth.isAuthenticated) {
                newProps.navigation.dispatch(NavigationActions.reset({ index: 0, actions: [NavigationActions.navigate({ routeName: 'Dashboard' })] }))
            }
        }, 5)
    }
        ...
        return (
            <NB.Container>
                <NB.Card style={styles.card}>
                    ...
                    <NB.Button style={styles.button} onPress={() => this.onButtonPress()} ><Text> SIGN IN </Text></NB.Button>
                </NB.Card>
            </NB.Container>
        )
    }
